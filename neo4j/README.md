...
**Nodes**
- `Depot {depot_id, name, country_code, depot_type, capacity_per_day}` — a resupply point. `depot_type` is one of `SUPPLY_DEPOT`, `PORT`, `AIRBASE`, `FORWARD_OPERATING_BASE`.
...
Schema, seed data, and read queries only — no application layer yet. The original `E-Commerce` prototype had a Next.js/React UI (`SupplyPlanning.tsx`, what-if capacity sliders); that's deliberately not ported here yet, since this repo has no frontend at all currently and the LangGraph/MCP roadmap items (see `CLAUDE.md`) may end up being how this data actually gets exposed, rather than a hand-built dashboard. Revisit once those layers exist.
